...
export default function Dashboard() {
  ...
  const statusColor = h.status === 'fresh' ? 'badge-green' : h.status === 'stale' ? 'badge-amber' : 'badge-red';

  return (
    <div>
      <div className="page-header">
        <h1>Dashboard</h1>
        <p>Your competitive intelligence overview — live market signals at a glance</p>
      </div>
      ...
      <div className="stat-grid">
        <div className="stat-card">
          ...
          <div className="stat-value">{health.loading ? '–' : (h.products_tracked ?? '–')}</div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ fontSize: 22, marginTop: 10 }}>
            <span className={`badge ${statusColor}`}>{h.status || 'loading...'}</span>
          </div>
          <div className="stat-sub">{h.last_updated ? new Date(h.last_updated).toLocaleString() : '–'}</div>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ fontSize: 15, marginTop: 10 }}>
            {product.loading ? '–' : (p.title?.slice(0, 30) + '...' || p.asin || '–')}
          </div>
          <div className="stat-sub" style={{ fontFamily: 'monospace', fontSize: 11 }}>{p.asin || ''}</div>
        </div>
      </div>
      ...
      <div className="glow-banner">
        <div>
          ...
          <p>Run all three AI agents and generate strategy cards in one click.</p>
        </div>
        <Link to="/strategy">
      ...
      {!product.loading && p.asin && (
        <div className="product-hero">
          <div className="product-info">
            ...
            {p.brand    && <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>Brand: {p.brand}</div>}
            {p.price    && <div style={{ fontSize: 14, color: 'var(--accent-light)', marginTop: 8 }}>₹{p.price}</div>}
            {p.rating   && <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>★ {p.rating} ({p.review_count} reviews)</div>}
          </div>
        </div>
      )}
      ...
      {!comps.loading && cs.length > 0 && (
        <div className="card">
              ...
              <tbody>
                {cs.slice(0, 5).map((c, i) => (
                  <tr key={c.competitor_asin || i}>
                    <td><span style={{ fontFamily: 'monospace', fontSize: 12, color: 'var(--accent-light)' }}>{c.competitor_asin}</span></td>
                    <td><strong>{c.product?.title?.slice(0, 45)}{c.product?.title?.length > 45 ? '…' : ''}</strong></td>
                    <td>{c.product?.brand || '–'}</td>
                    <td className="comp-price">{c.latest_price ? `₹${c.latest_price}` : '–'}</td>
          ...
          {cs.length > 5 && (
            <Link to="/competitors" style={{ display: 'inline-block', marginTop: 14, fontSize: 13, color: 'var(--accent-light)' }}>
